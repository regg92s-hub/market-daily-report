...
OUT_REPORT_JSON = PAGES_DIR / "report.json"
OUT_REPORT_MD = PAGES_DIR / "report.md"
OUT_REPORT_TABLE = PAGES_DIR / "report_table.html"
...
def _frame_summary(fr):
    ...
    return {
        ...
        "dist_to_36MA": (last - sma36) / sma36 if (isinstance(last,(int,float)) and isinstance(sma36,(int,float)) and sma36) else None,
        "rsi14": fr.get("rsi14"),
        "macd": fr.get("macd"),
...
def _asset_summary(ticker, a):
    out = {
        ...
        # uketeller om den finnes i index.json; ellers None
        "weekly_close_count_above_36WMA": a.get("weekly_close_count_above_36WMA"),
        # miners-flagg hvis du allerede mater disse inn i index.json
...
def build_table_html(assets, generated_local):
    ...
    <tr>
      ...
      <td>{'Ja' if d.get('close_above_sma36') is True else ('Nei' if d.get('close_abma36') is False else ('Ja' if d.get('close_above_sma36') else ''))}</td>
...
def maybe_inject_table_into_index_html(table_html):
    if not INDEX_HTML.exists():
        return
    html = INDEX_HTML.read_text(encoding="utf-8")
    # Sett inn tabellen rett før "Grafer"-seksjonen hvis mulig, ellers på slutten
    if "</body>" in html:
        if "<h2>Grafer</h2>" in html:
            html = html.replace("<h2>Grafer</h2>", table_html + "\n<h2>Grafer</h2>")
        else:
...
def main():
    if not INDEX_PATH.exists():
        raise SystemExit(f"Mangler {INDEX_PATH}")
    ...
    with open(OUT_REPORT_JSON, "w", encoding="utf-8") as f:
        json.dump({
            ...
            "spec_version": "v1",
            "assets": assets_out,
            "news_source_present": NEWS_PATH.exists()
        }, f, ensure_ascii=False, indent=2)

    # report.md
    lines = [
        f"Daglig rapport (kompakt) – {generated_local}",
        "",
    ...
    OUT_REPORT_TABLE.write_text(table_html, encoding="utf-8")

    # valgfritt: injiser tabellen inn i forsiden
    maybe_inject_table_into_index_html(table_html)
